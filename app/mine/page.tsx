        ...
        <div className="py-8 px-3 flex flex-col gap-5 bg-white rounded-t-3xl min-h-screen">
          ...
          {selectedCategory === 1 && !isPending && !error && (
            <div className="flex flex-col gap-1 pb-20">
              {data
                .filter((tapper: profitType) => tapper.field === "Tappers")
                .map((item: profitType) => {
                  return (
                    <MineCard
                      key={`card-${item.name}`}
                      name={item.name}
                      image={item.photo}
                      goldProfit={item.gold_profit_per_click}
                      greenProfit={item.green_profit_per_click}
                      price={item.price}
                      upgradePeriod={item.upgrade_period}
                      receive={item.receive}
                      goldProfitPerHour={item.gold_profit_per_hour}
                      greenProfitPerHour={item.green_profit_per_hour}
          ...
          {selectedCategory === 0 && !isPending && !error && (
            <div className="flex flex-col gap-1 pb-20">
              {data
                .filter((tapper: profitType) => tapper.field === "Holders")
                .map((item: profitType) => {
                  return (
                    <MineCard
                      key={`card-${item.name}`}
